...
app.get('/listado-productos', function(req, res){
    //Render using EJS
...
app.get('/calorias', function(req, res){
    //Render using EJS
    res.render('calorias', {
        title: 'Senpai! Calorias',
        mensaje: ''
    });
});
...
app.post('/calorias', function(req, res){
    let mensaje = '';
    let totalSemana = parseInt(req.body.sem1) + parseInt(req.body.sem2);
    
    if(totalSemana > 500){
        mensaje = 'Estas habilitado';
    } else {
        mensaje = 'No estas habilitado';
    }
